...
SELECT 
    p.player_id, p.name AS `player_name`, SUM(rp.player_end_points - rp.player_start_points) AS `earnings`, COUNT(DISTINCT rp.game_id) AS `games_played`,
    SUM(TIMESTAMPDIFF(MINUTE, g.start_time, g.end_time)) AS `minutes_played`
FROM `players` p
